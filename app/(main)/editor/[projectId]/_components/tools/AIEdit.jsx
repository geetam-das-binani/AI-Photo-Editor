...
const AIEdit = ({ project }) => {
  ...
  const applyRetouch = async () => {
    ...
    try {
      ...
      const retouchUrl = buildRetouchUrl(currentImageUrl, selectedPreset);
      const retouchImage = await FabricImage.fromURL(retouchUrl, {
      ...
      retouchImage.set(imageProps);
      canvasEditor.add(retouchImage);
      retouchImage.setCoords();

      canvasEditor.setActiveObject(retouchImage);
      canvasEditor.requestRenderAll();
      await updateProject({
        projectId: project._id,
        currentImageUrl: retouchUrl,
        canvasState: canvasEditor.toJSON(),
        activeTransformations: selectedPresetData.transform,
      });
      toast.success(
        `Image enhanced with ${selectedPresetData.label} successfully`
      );
    } catch (error) {
      console.error("Error applying retouch:", error);
      toast.error(`Failed to enhance image with ${selectedPresetData.label}`);
    } finally {
      setProcessingMessage("");
    }
  };

  function buildRetouchUrl(imageUrl, presetKey) {
